...
function defaultState() {
  const games = HISTORY.map(g => ({
    date: '', kyleScore: g.kyleScore, nicoleScore: g.nicoleScore,
    winner: g.winner, rounds: 0, dealer: null,
  }));
  return {
    kyleWins: 17, nicoleWins: 24,
    kyleTotalPts: 4169, nicoleTotalPts: 3905,
    games, rounds: [], firstDealer: null, active: 'kyle',
  };
}
...
/* ── Dealer helpers ──
   ...
   - Round 1 → firstDealer
   - Round 2 → other person
   - Round 3 → firstDealer  ... and so on
   currentDealer() always looks at how many COMPLETE rounds exist.
   The moment Nicole's score is saved, that round is complete,
   so the next call to currentDealer() returns the other person. ── */
function dealerForRound(roundIndex) {
  if (!S.firstDealer) return null;
  const both = ['kyle', 'nicole'];
  return both[(both.indexOf(S.firstDealer) + roundIndex) % 2];
}

function currentDealer() {
  // Number of fully completed rounds = index of the NEXT round to be dealt
  return dealerForRound(completeRounds().length);
}
...
function selectPlayer(p) {
  if (editMode) return;
  S.active = p; kpVal = ''; save();
  updateKPDisplay(); updateLabel();
...
function kpEnter() {
  ...
  if (S.active === 'kyle') {
    ...
    // Nicole's score saved → round is complete → dealer rotates automatically
    S.rounds[S.rounds.length - 1].nicole = v;
    S.active = 'kyle';
  }
  kpVal = ''; save(); updateAll();
...
function startEdit(roundIdx, player) {
  ...
  document.getElementById('kp-who').innerHTML =
    `Editing R${roundIdx + 1} · <span style="color:#BA7517">${player === 'kyle' ? 'Kyle' : 'Nicole'}</span>`;
...
function cancelEdit() {
  editMode = null; kpVal = '';
  updateKPDisplay(); updateLabel(); exitEditUI();
}
...
function updateLabel() {
  if (editMode) return;
  const pendingNicole = S.rounds.length > 0 && S.rounds[S.rounds.length - 1].nicole === null;
  const dispRnd = pendingNicole ? S.rounds.length : completeRounds().length + 1;
  document.getElementById('kp-who').innerHTML =
...
function updateDealerBar() {
  ...
  if (!S.firstDealer) { bar.classList.remove('show'); return; }
  bar.classList.add('show');
  const dealer = currentDealer();
  document.getElementById('dealer-name').textContent = dealer === 'kyle' ? 'Kyle' : 'Nicole';
  document.getElementById('card-kyle').classList.toggle('dealing',   dealer === 'kyle');
  document.getElementById('card-nicole').classList.toggle('dealing', dealer === 'nicole');
}
...
function updateAll() {
  ...
  function setTag(id, val) {
    const el = document.getElementById(id), rem = 100 - val;
...
function renderRounds() {
  ...
  tbody.innerHTML = S.rounds.map((r, i) => {
    ...
    const kl = complete && kr < nr, nl = complete && nr < kr;
    const kScore = complete ? r.kyle   : '?';
    const nScore = complete ? r.nicole : '—';
    const kRem = complete ? 100 - kr : '', nRem = complete ? 100 - nr : '';
    ...
    const d = dealerForRound(i);
    const dealerTag = d ? `<span class="round-dealer">${d === 'kyle' ? 'K' : 'N'}🃏</span>` : '';
    return `<tr class="${isEdit ? 'editing-row' : ''}" id="rrow-${i}">
      <td>R${i + 1}${dealerTag}</td>
      <td class="${kl ? 'lead' : ''} ${kEdit ? 'pending-cell' : ''}" onclick="startEdit(${i},'kyle')">${kScore}</td>
      <td class="${kSubClass}">${kRemText}</td>
      <td class="${nl ? 'lead' : ''} ${nEdit ? 'pending-cell' : ''}" onclick="${complete ? `startEdit(${i},'nicole')` : 'void(0)'}">${nScore}</td>
...
function triggerWin(kt, nt) {
  ...
  S.kyleTotalPts += kt; S.nicoleTotalPts += nt;
  S.games.push({ date: todayStr(), kyleScore: kt, nicoleScore: nt, winner, rounds: completeRounds().length, dealer: S.firstDealer });
  // Whoever dealt the last round → other person deals round 1 next game
  S._nextFirstDealer = dealerForRound(completeRounds().length - 1) === 'kyle' ? 'nicole' : 'kyle';
  const nextName = S._nextFirstDealer === 'kyle' ? 'Kyle' : 'Nicole';
  ...
  document.getElementById('next-dealer-note').innerHTML =
    `<strong>${nextName}</strong> deals round 1 of the next game`;
  save(); renderRecord();
...
function startNewGame() {
  S.firstDealer = S._nextFirstDealer || (S.firstDealer === 'kyle' ? 'nicole' : 'kyle');
  delete S._nextFirstDealer;
  S.rounds = []; S.active = 'kyle'; kpVal = ''; editMode = null;
  save(); closeModal('win-modal'); stopConfetti(); exitEditUI(); updateAll();
}

function setFirstDealer(p) {
  S.firstDealer = p; save(); closeModal('dealer-modal'); updateDealerBar();
}

function launchConfetti() {
  const canvas = document.getElementById('confetti-canvas');
  const wrap   = document.getElementById('main-wrap');
  canvas.width = wrap.offsetWidth || 380; canvas.height = wrap.offsetHeight || 600;
  ...
  confettiPieces = Array.from({ length: 130 }, () => ({
    ...
    r: 5 + Math.random() * 6, d: Math.random() * 80 + 60,
    color: colors[Math.floor(Math.random() * colors.length)],
    tiltAngle: 0, tiltSpeed: .05 + Math.random() * .1,
    speed: 2 + Math.random() * 3, rotation: Math.random() * 360,
    rotSpeed: (Math.random() - .5) * 6, shape: Math.random() > .5 ? 'rect' : 'circle',
  }));
  let tick = 0;
  function draw() {
    ...
    confettiPieces.forEach(p => {
      ...
      ctx.restore();
      p.y += p.speed; p.x += Math.sin(tick * .02 + p.d) * 1.2;
      p.rotation += p.rotSpeed; p.tiltAngle += p.tiltSpeed;
    });
    tick++;
...
function renderHistory() {
  ...
  if (!S.games || S.games.length === 0) { el.innerHTML = '<div class="empty">No games yet</div>'; return; }
  el.innerHTML = [...S.games].reverse().map(g => {
    const isKyle = g.winner === 'Kyle', isNicole = g.winner === 'Nicole';
    ...
    const meta = [g.date || null, g.rounds ? g.rounds + ' rounds' : null].filter(Boolean).join(' · ');
...
function doReset() {
  S.rounds = []; S.active = 'kyle'; kpVal = ''; editMode = null;
  save(); closeModal('reset-modal'); stopConfetti(); exitEditUI(); updateAll();
}
